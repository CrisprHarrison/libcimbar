var Main = function () {
  ...
  return {
    ...
    clickNav: function () {
      // No nav button in fixed sidebar design
    },
...
window.addEventListener('keydown', function (e) {
  ...
  if (e.target instanceof HTMLBodyElement) {
    if (e.key == 'Enter' || e.keyCode == 13 ||
      ...
      // Focus on nav content for fixed sidebar
    ...
    if (e.key == 'Escape' || e.keyCode == 27 ||
      ...
      for (var i = 0; i < links.length; i++) {
        if (links[i].classList.contains('attention')) {
          var next = i + 1 == links.length ? links[0] : links[i + 1];
          links[i].classList.remove('attention');
          next.classList.add('attention');
          return;
        }
      }
      // If no attention class, add to first
      if (links.length > 0) {
      ...
      for (var i = 0; i < links.length; i++) {
        if (links[i].classList.contains('attention')) {
          var next = i == 0 ? links[links.length - 1] : links[i - 1];
          links[i].classList.remove('attention');
          next.classList.add('attention');
          return;
        }
      }
      // If no attention class, add to last
      if (links.length > 0) {
